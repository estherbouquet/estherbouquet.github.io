...
class Wiggler {
  ...
  Wiggler() {
    ...
    for (float a = 0; a < TWO_PI; a+=0.2) { // changer valeur de a = changer la forme
  ...
  void wiggle() {
    ...
    for (int i = 0; i < s.getVertexCount(); i++) {
      ...
      xoff+= 0.5; // plus la valeur est petite plus les bords sont arrondis
    }
    // Increment perlin noise y value
    yoff += 0.01; // plus la valeur est petite plus les mouvements sont lents
  }  
